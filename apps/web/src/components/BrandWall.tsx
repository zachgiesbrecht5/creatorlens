            ...
            <div key={c.brand_id} className={`card relative p-5 transition ${isOpen ? "border-fg" : "hover:border-dim"}`}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <Link href={`/brands/${c.brand_id}`} className="block truncate text-lg font-semibold hover:underline" onClick={(e) => e.stopPropagation()}>{c.brand}</Link>
                  <div className="mt-1 font-mono text-[10px] text-muted">
                    ...
                    {Number(c.creators_booked) > 1 && <> · books {c.creators_booked}</>}
                  </div>
                </div>
